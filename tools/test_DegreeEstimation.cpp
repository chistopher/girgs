...
int main(int argc, char* argv[]) {
    ...
    test(abs(short_edges - test_short) < epsilon);
    test(abs(long_edges - test_long) < epsilon);
    test(abs(edges - test_edges) < epsilon);

    test(0.99*edges < experimental_number_of_edges);
    test(1.01*edges > experimental_number_of_edges);
