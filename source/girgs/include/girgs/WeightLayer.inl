...
WeightLayer<D>::WeightLayer(unsigned int layer, unsigned int targetLevel,
                            const SpatialTreeCoordinateHelper <D> &helper, std::vector<int> &&nodes,
...
, m_target_level(targetLevel) // w0*wi/W = 2^(-dl) solved for l --- l = (log2(W/w0^2) - i) / d
{
    ...
    // allocate stuff
    auto points_in_cell = std::vector<int>(cellsInLevel, 0);
    m_prefix_sums.resize(cellsInLevel+1, 0);
    m_A.resize(nodes.size());

    // count num of points in each cell
    auto cellForPoint = std::vector<unsigned int>(nodes.size(), -1);
    for (int i = 0; i < nodes.size(); ++i) {
        auto targetCell = helper.cellForPoint(positions[nodes[i]], m_target_level);
        cellForPoint[i] = targetCell; // remember this for last loop
        ++points_in_cell[targetCell];
    }
    ...
    for(auto cell = 1u; cell <= cellsInLevel; ++cell) {
        m_prefix_sums[cell] = m_prefix_sums[cell-1] + points_in_cell[cell-1];
    }
    ...
    for(int i = 0; i < nodes.size(); ++i){
        ...
        auto nodeId = nodes[i];
        m_A[m_prefix_sums[targetCell] + num_inserted[targetCell]] = Node<D>(positions[nodeId], weights[nodeId], nodeId);
...
int WeightLayer<D>::pointsInCell(unsigned int cell, unsigned int level) const {
    ...
    return m_prefix_sums[end] - m_prefix_sums[begin];
}


template<unsigned int D>
const Node<D>& WeightLayer<D>::kthPoint(unsigned int cell, unsigned int level, int k) const {
    using Helper = SpatialTreeCoordinateHelper<D>;
    assert(level <= m_target_level);
...
template<unsigned int D>
const Node<D>* WeightLayer<D>::firstPointPointer(unsigned int cell, unsigned int level) const
{
	using Helper = SpatialTreeCoordinateHelper<D>;
	assert(level <= m_target_level);
	assert(Helper::firstCellOfLevel(level) <= cell && cell < Helper::firstCellOfLevel(level + 1)); // cell is from fromLevel
