...
static std::vector<uint64_t> count_edges(const std::vector<double>& radii, const std::vector<double>& angles,
                                         double T, double R, int seed, const unsigned num_graphs) {
...
static uint64_t count_violations(const std::vector<double>& radii, const std::vector<double>& angles, double T, double R,
    ...
    for(int u = 0; u < n; ++u) {
        for(int v = u + 1; v < n; ++v){
            ...
            const double pvalue = 2.0 * ((count < mean)
                ? cdf(binomial(num_graphs, prob), count)
                : cdf(complement(binomial(num_graphs, prob), count - 1)));

            EXPECT_GT(pvalue, confidence)
                << "Edge (" << u << ", " << v << ") with r1=" << radii[u] << ", phi1" << angles[u]
                << ", r2=" << radii[v] << ", phi2" << angles[v]
                << ", dist=" << dist << ", prob=" << prob << ", count=" << count << ", p-value=" << pvalue;
 ...
 * number N(u,v) of times it is generated. Given that (u,v) as a probability of p(u,v) we expect N(u,v) to be distributed as
 * Bernoulli(numGraphs, p(u,v)). We compute its p-value and reject with a significance level of sig (corrected for the fact,
 * the we carry out (n over 2) trials).
 */
